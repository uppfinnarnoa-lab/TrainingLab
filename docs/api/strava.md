...
**Side effects:** Drives `backfillRunner` (in-process singleton, keyed by `userId`), which paginates through the user's full Strava history and fetches per-activity detail, persisting progress so it can resume after a rate limit or process restart. Since 2026-06-26 also fetches and caches each activity's full stream (`backfillOneActivityStream()`), two requests per activity instead of one.

**Gating bug fixed 2026-06-26:** the pending-activity query was `splitDetailFetched: false` only — correct before streams were added to this job, but meant a user who'd already completed a full backfill (virtually everyone with an established account) would have *nothing* match the query ever again, even though none of their activities had a cached stream. Now `OR: [{ splitDetailFetched: false }, { stream: null }]`, and each fetch step inside the loop is conditional on what that specific activity is actually still missing (no redundant detail re-fetch for an activity that only needs its stream).
