...
*Last updated: 2026-06-30 — Finished the Planner + Dashboard/stats-chart color-consistency fix (real Settings colors instead of a hardcoded palette) that 2026-06-28d left as remaining scope, plus closed two call sites that session missed; built Google Calendar's dedicated-calendar layer (narrower `calendar.app.created` scope) and nearest-match event coloring on top.*
