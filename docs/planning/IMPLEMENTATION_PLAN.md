...
*Last updated: 2026-06-28c — Diagnosed why Activity History race/workout colors stay stale on production (backfill button can't reach already-detail-fetched activities; the corrective script was only ever run on local dev) and fixed the Garmin backfill route re-fetching already-synced days from a bot-sensitive API.*
