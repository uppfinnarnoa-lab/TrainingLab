...
*Last updated: 2026-06-28d — Implemented Settings-backed color resolution for Activities/History (FRONTEND_UX_AUDIT_PLAN §4.6): races, running sub-types, and orienteering (detected by name, since Strava has no real OL sport_type) now use the user's real configured colors instead of a hardcoded palette; Planner and the Dashboard/stats/volume charts still use the old palette and remain follow-up work.*
