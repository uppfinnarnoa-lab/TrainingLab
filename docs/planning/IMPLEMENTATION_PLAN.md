...
**Bug 15 — user's own hypothesis, investigated and confirmed correct: "is the OL filter being missed?"**

Found a real, structural gap in the orienteering exclusion logic, present in **both** `windowOlFilter` (`updateVO2maxAndPaces`) and `olRaceFilterLight` (`updateHRZones`): the pace-threshold check (excluding anything slower than the bootstrapped easy-training pace — terrain/navigation-limited, not fitness-limited) only applied `if (a.isRace)`. A slow OL **training** session — not marked as a race, and without a name containing one of the hardcoded keywords (`ol`, `orienteringsl`, `skogsl`, `olpass`, `orienteer`, `moc`, `stafett`) — passed through completely unfiltered, regardless of how slow/terrain-limited it actually was.

This asymmetrically explains why only the *laps-only* computation (Bug 14) degraded so much more than the combined one: a single such leaked OL session contributes exactly one data point to the whole-activity dataset, but potentially 10-20+ individual lap data points to the laps-only dataset (one per auto-lap), giving it dramatically more pull on that regression specifically.

**Fix:** removed the `isRace` restriction in both places — the pace-threshold check now applies to every activity that passes the name filter, race or not (`a.averageSpeed == null || pace < threshold`, defaulting to *included* only when speed is genuinely unknown).

**Re-confirmed, since the user raised it again:** Garmin cannot be involved in any of this. Grepped every `prisma.activity.*` write across the whole codebase — all of them are Strava sync/backfill/weather-backfill or planner-match routes; `lib/garmin/sync.ts` only ever writes `GarminDailySummary` rows. There is no code path by which Garmin data reaches the `Activity` table that this entire pace/HR regression is built from.

**Verification:** `pnpm build --no-lint` passes. Not re-verified against the user's real data (no production DB access) — the user will need to clear `FitnessCache` and re-sync once more after this deploy to see the full effect.

---

*Last updated: 2026-06-18 (coach chat streaming/thinking-indicator fix + tool fetch timeouts, large-screen layout fix, planner desktop drag-and-drop: PointerSensor restore + native/dnd-kit conflict removed from WorkoutPill + display:contents collision-detection fix, laps/splits chart outlier+offset scaling fix, rolling calendar fills available height, statistical-threshold-estimation cache-overwrite fix, LT-trend halfLife cliff-edge smoothing + symmetric rate cap, lap-aware zone-time classification, statistical-threshold card now mirrors applied zones + create-branch field fix, duplicate HR-zone-distribution title disambiguated, info-tooltip hover-flicker fix, LT trend now uses combined activities+laps data matching the real estimator, OL exclusion gap closed for non-race training sessions)*
