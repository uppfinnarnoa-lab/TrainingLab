...
- 40×40 SVG-`viewBox`. Formen är bokstaven **T**, byggd av en enda `path`: `M2,7 H38 V17 H27 V35 H13 V17 H2 Z` (`components/logo.tsx:33`) — ett 36px brett, 10px högt tak (x=2–38, y=7–17) och en 14px bred, 18px hög stam (x=13–27, y=17–35). Stamen slutar vid y=35 (87,5 % av viewBox-höjden). De återstående 5 enheterna (y=35–40) är tomt utrymme som representerar descender-zonen — se 8.7.3 för varför detta värde ger korrekt baslinjejustering.
- En **inskuren vågform** ("aktivitetspuls", EKG-liknande) klipps ut ur formen via en SVG-`mask` (`logo.tsx:19-31`) — en `polyline` (`points="7,25 15,25 18,20 21,31 24,25 33,25"`, `stroke-width="3"`, `stroke-linecap="butt"`, `stroke-linejoin="miter"`) med baslinje y=25 (höjd jämfört med tidigare y=27.5), topp y=20 och trough y=31 — 6 raka segment, inga knutar. Vågformen löper genom övre delen av stammen och sticker ut genom båda sidorna. Detta är den enda grafiska detaljen utöver bokstavsformen: ett T som samtidigt läses som ett pulsslag/aktivitetsdiagram, en direkt visuell koppling till appens syfte. *(Uppdaterad 2026-07-01: vågformen höjd och gjord mer regelbunden per designgranskning — se session 2026-07-01b i IMPLEMENTATION_PLAN.md.)*
...
- **Baslinje-justering (fixad 2026-07-01, slutlig lösning):** Lösningen är i två delar beroende på sammanhang:
  - **`LogoWordmark` (login/register-sidor):** Renderas nu som ett enda kombinerat SVG-element (`logo.tsx:52-88`) istället för en flex-behållare med separata element. SVG-text placeras med `y=35` (baslinje i SVG-koordinater) och T-stamen slutar vid `y=35` — de delar identisk baslinje *per konstruktion*, inga CSS-mätningar involverade. Pixelverifierat med Playwright: T-botten och text-baslinje båda vid exakt samma px-position.
  - **Sidopanel (`Sidebar`, `Logo` + `LogoText` i flex-rad):** T-path med `y=35` ger T:ets visuella underkant vid 87,5 % av SVG-höjden. Med `align-items:flex-end` hamnar T-botten ~0,3 px från text-baslinjen — sub-pixelnivå, osynligt. Inga extra marginaler behövs.
  - Tidigare felaktiga försök (2026-06-30: `marginBottom = size × 0.05`; 2026-07-01 tidigt: `V40`) ökade gapet — de återkallades. Rotorsaken var att `getBoundingClientRect` på path-elementet returnerar hela SVG-rektangeln, inte T:ets visuella underkant, vilket ledde till felaktig analys.
