...
- 40×40 SVG-`viewBox`. Formen är bokstaven **T**, byggd av en enda `path`: `M2,7 H38 V17 H27 V38 H13 V17 H2 Z` (`components/logo.tsx:33`) — ett 36px brett, 10px högt tak (x=2–38, y=7–17) och en 14px bred, 21px hög stam (x=13–27, y=17–38).
...
- **Baslinje-justering (fixad 2026-06-30):** T-stammens underkant slutar vid y=38 i en 40-enheters viewBox — 5 % visuellt tomrum under T:et. Med enbart `align-items:flex-end` hamnar T:ets visuella underkant *under* textens typografiska baslinje (i "g"-descender-zonen), vilket gör att T:et ser ut att sjunka under resten av ordet. Fix: `marginBottom = size × 0.05` på SVG-ikonen i `LogoWordmark` (`logo.tsx:78`) och i sidopanelens expanded-state (`sidebar.tsx:83`). Det externa marginal-utrymmet skjuter ned flex-behållarens botten, vilket kompenserar exakt för de 5 % — T:ets underkant och textens baslinje (r, a, i, n, L, a, b) delar samma "golv".
