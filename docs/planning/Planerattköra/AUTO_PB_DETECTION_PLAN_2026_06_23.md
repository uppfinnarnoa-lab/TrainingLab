...
## 2b. KRITISK RISK (tillagd vid granskning): samma backfill-flood-problem som pass-sammanfattningsfunktionen

Exakt samma risk som identifierades i [[POST_WORKOUT_AI_SUMMARY_PLAN_2026_06_23]] §2b gäller här, med ett annat symptom: om "Automatisk" slås på och `onNewActivityCreated` (eller en första full Strava-synk) sedan bearbetar års gammal historik, skulle PB-trackern plötsligt svämmas över med auto-skapade `RaceRecord`-rader från gamla träningssegment — inte bara genuint nya pass. Värre än notis-spammet i den andra planen, eftersom resultatet (felaktiga/oönskade `RaceRecord`-rader) **kvarstår permanent i datan** tills användaren manuellt städar bort dem, snarare än att bara vara en notis som kommer och försvinner.

**Samma spärrprincip, samma fält om möjligt:** detektion ska bara köras för aktiviteter vars `startDate` ligger efter den tidpunkt då `pbDetectionMode` senast sattes till `"automatic"` (spara `pbDetectionModeChangedAt: DateTime?` på `AthleteProfile`, eller återanvänd om en motsvarande tidsstämpel redan infördes av [[POST_WORKOUT_AI_SUMMARY_PLAN_2026_06_23]] i samma implementationsomgång — bygg INTE två separata "när aktiverades funktionen"-tidsstämplar om en delad mekanism räcker).

**Komplement, inte ersättning — en explicit, separat "Skanna historik efter PB:n"-knapp:** många användare vill sannolikt FÅ sin historiska bästa-data in i trackern, bara inte som en överraskande bieffekt av att slå på en växel. Lägg till en fristående, användarinitierad bulk-action (t.ex. på Races-sidan: "Skanna alla tidigare pass efter personbästa") som explicit kör samma matchningslogik (§3/§4) över ALL historik på en gång, som ett medvetet, granskningsbart engångskommando — separat från, och oberoende av, den löpande automatiska detekteringen på nya pass. Detta löser samma underliggande behov utan flood-risken: ett medvetet klick istället för en tyst bakgrundseffekt av en inställningsändring.
...
- `app/(dashboard)/races/races-client.tsx` — "Skanna historik efter PB:n"-knapp (§2b)
- `docs/api/races.md` — dokumentera den nya sidoeffekten ("ny `RaceRecord` kan skapas automatiskt vid sync, se `pbDetectionMode`") + den nya bulk-scan-endpointen
