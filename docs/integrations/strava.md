...
- **Sync**: Initial full history (paginated, 200/page). Daily incremental cron at 06:00. Historical backfill (split detail + stream) resumes automatically each night at 00:30 UTC until complete — also fetches and caches each activity's full stream (added 2026-06-26). Manual trigger via UI. The cron job and the manual UI trigger now share one concurrency guard (`backfillRunner.startIfIdle()`). A daily-limit hit ends the run immediately instead of blocking for up to 24h — the next cron tick or manual restart picks up where it left off.
