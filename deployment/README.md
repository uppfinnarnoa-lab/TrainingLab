...
Claude cannot reach this server (port 22 is unreachable from the dev machine) — at the end of each session it hands you a command to run yourself over your own SSH session.

**Default — most updates (no dependency or schema changes):**
```bash
...
- Running the longer command when nothing changed is harmless (the install/prisma steps become no-ops), just slower.
